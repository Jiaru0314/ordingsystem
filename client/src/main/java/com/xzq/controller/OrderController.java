...
import com.xzq.entity.*;
import com.xzq.feign.OrderFeign;
...
public class OrderController {
    ...
    public String save(@PathVariable("mid") long mid, HttpSession session) {
        ...
        Menu menu = new Menu();
        menu.setId(mid);
        order.setUser(user).setMenu(menu).setDate(new Date());
        orderFeign.save(order);
        return "redirect:/base/redirect/order";
    }
    ...
    public String updateState(@PathVariable("id") long id, @PathVariable("state") int state, HttpSession session) {
        ...
        return "redirect:/base/redirect/order_handler";
    }
}
